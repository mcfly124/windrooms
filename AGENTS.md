...
`/book` is the Windrooms landing page (design handoff in ~/Downloads/design_handoff_windrooms_booking — high-fidelity, EN/PL copy final). Own look, NOT the app design system: `book.css` (.bk- prefix, navy #1A2E40 / orange #C96F4A / Instrument Serif + Space Grotesk via next/font in `book/layout.tsx`), always light. `Landing.tsx` = single-page flow: hero (parallax), room-type cards (prices + counts live from DB), 4-step booking (range calendar → type with live availability → guest details → confirmation showing the FR-xxxxx reference; no card fields — Stripe checkout slots in later). Books via `createPublicBookingByType` (assigns first free room of the type; all rules apply: `dayOpenToPublic` planner overrides + `releaseWindowDays`, credit-holder block, auto EXTERNAL client, confirmation email). lang/currency persist in localStorage; EUR uses the live NBP rate. Photos: `/public/book/{hero,single,double,location}.jpg` — placeholder boxes until supplied. Manage entry: footer link → `/book/manage`. TBD from owner: exact address, contact email/phone in the footer/location copy.
