...
/**
 * Per-day count of bookable rooms (any type) at Gdańsk for a whole month, so the
 * booking calendar can grey out days with nothing free. A day counts a room when it
 * is not held by a CONFIRMED reservation that night and the day is open to the public.
 */
export async function gdanskMonthAvailability(
  year: number,
  month: number // 1-12
